...
can lead to an eponential blow-up. To face the state explosion
...
Binary Decision Diagram or BDD.
...
because when applied on closed set and a partial order,
antichains and the associated operations such as the lower
and upper closure allow to retrieve more elements from the
closed set. We define formally
...
\subsection{A concrete example: Universality problem}
...
\todo{talk about subset construction}
...
maximal set of states is sufficient. Computing the workd $1^k$,
will lead to a maximal path for this automaton. For any word $w < 1^k$
of size $k$,
...
that is $f_{1^k} \cap F = \emptyset$ then for the word $w$,
since $f_w \subseteq f_{1^k}$, it holds that $f_w \cap F = \emptyset$.
...
\todo{Tree vs Bitarray representations}
...
\todo{Write a paragraph about why Java and not C}
