...
\todo{Either put this here in the intro, or in the next year section}
...
shows a graph with the uncomparable elements of such poset.
...
\begin{figure}
    ...
    to $Q_A = \{q_0, q_1, q_2\}$. Each arrow of the graph
    correspond to a valid comparison using the set inclusion. For example
    $\{q_0\} \rightarrow \{q_0, q_1\}$ corresponds to
     the inclusion $\{q_0\} \subseteq \{q_0, q_1\}$. Two elements of the graph
     with no connection, means that the elements are uncomparable. For
     example, $\alpha = \{\{q_0\}, \{q_1\}, \{q_2\}\}$ is an antichain.}
